...
def evaluate_conceptual_features(board: np.ndarray) -> np.ndarray:
    ...
    # New: Add a "difficulty" feature based on gaps and max height
    difficulty_score = (gaps * 0.5) + (max_height * 0.3) + (board_fullness * 0.2)
    
    return np.array([lines_cleared, gaps, max_height, board_fullness, difficulty_score], dtype=np.float32)
...
class MemoryEfficientDataset(Dataset):
    """
    A dataset class that generates Tetris game data on the fly, with new features
    ...
    def __getitem__(self, idx) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, Dict]:
        ...
        # New: Add a flag for adversarial data
        is_adversarial = conceptual_features[1] > 5 and conceptual_features[2] < 10 # Heuristic: many gaps but low height
        ...
        contextual_data = {
            ...
            "conceptual_features_names": ['lines_cleared', 'gaps', 'max_height', 'board_fullness', 'difficulty_score'],
            "explanation": "This move fills a large gap and prepares for a line clear.",
            "difficulty_score": conceptual_features[-1]
        }

        return tensor_before_binary, board_target_binary, next_piece_tensor, conceptual_tensor, contextual_data
